...
Puppet::Type.type(:rabbitmq_user).provide(:rabbitmqctl) do
  ...
  def create
    if resource[:passwordfile]
      # Read password from file and place into :password
      password = File.open(resource[:passwordfile], 'rb').read.strip
    else
      password = resource[:password]
    end
    rabbitmqctl('add_user', resource[:name], password)
